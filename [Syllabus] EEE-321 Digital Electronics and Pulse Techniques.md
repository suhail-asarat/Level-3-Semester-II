# Taohid Sir's Part  
### Text Books:  
- [R. P. Jain - Modern Digital Electronics [4th Edition] (2012)](https://t.me/c/1734256119/3086/3132)  
- [Robert L. Boylestad, Louis Nashelsky - Electronic Devices and Circuit Theory [11th Edition]](https://t.me/c/1734256119/3086/3133)  
- [Robert F. Coughlin, Frederick F. Driscoll - Operational Amplifiers and Linear Integrated Circuits](https://t.me/c/1734256119/3086/3177)  

---

> R. P. Jain - Modern Digital Electronics [4th Edition] (2012)  

## To Build Up Intuition (Not sure wheater in the Syllabus or not)   
### Chapter 03 : Semiconductor Devices - Switching Mode Operation  

3.3 p-n JUNCTION DIODE  
3.5 BIPOLAR JUNCTION TRANSISTOR  
3.7 FIELD-EFFECT TRANSISTOR  

---

> R. P. Jain - Modern Digital Electronics [4th Edition] (2012)  

## Chapter 04 : Digital Logic Families  

### Basics  

Implementation or Design of AND, NAND, NOR gate using Transistor and and Resistor  

**Things to know:** knee voltage, BJT-junction, cut-off region, CMOS, NMOS, PMOS  

### 4.3 Resistor-Transistor Logic (RTL)  

Fig. 4.4 - A 2-input RTL NOR Gate Driving N Similar Gates  

### 4.6 Diode-Tansistor Logic (DTL)  

Fig. 4.12 - A 3-Input DTL NAND Gate Driving N Similar Gates  

Example 4.1  

Amplitude Factor  

### 4.8 Transistor-Transistor Logic (TTL)  

Fig. 4.16 - A 3-Input TTL NAND Gate Driving N Similar Gates  

#### 4.8.7 Active Pull-up  

Fig. 4.18 - A TTL Gate with Totem-Pole Output Driver  
Corresponding Math  

---

> Robert L. Boylestad, Louis Nashelsky - Electronic Devices and Circuit Theory [11th Edition]  

## [Chapter 02 : Diode Application](https://t.me/c/1734256119/3086/3333)  

### 2.8 Clippers  

Definition  

#### Series  

Fig. 2.68, 2.69  
Example 2.18, 2.19  

#### Parallel  

Fig. 2.81  
Example 2.20, 2.21  

### 2.9 Clampers  

Definition  

Example 2.22, 2.23  

**YouTube Playlist:** [Clipper and Clamper](https://www.youtube.com/playlist?list=PLstSxcpBGrtVbt4A3Ns1Rse0IEhaPpzdU)  

---

> Robert F. Coughlin, Frederick F. Driscoll - Operational Amplifiers and Linear Integrated Circuits  

## Chapter 01 : Introduction to Op Amps  

Figure 1.1, 1.2, 1.3  


## Chapter 02 : First Experiences with an Op Amp  
### 2.2 Open-Loop Voltage Gain  

### Closed-Loop Gain -> Feedback Loop  


## Chapter 03 : Inverting and Noninverting Amplifiers  
### 3-1 The Inverting Amplifier  
#### 3-1.2 Positive Voltage Applied to the Inverting Input  

Figure 3-1  
Example 3-1  

### 3-2 Inverting Adder and Audio Mixer  
#### 3-2.1 Inverting Adder  

Figure 3-4  

### 3-5 Noninverting Amplifier  
#### 3-5.1 Circuit Analysis  

Figure 3-7  

**Practice Math:** with values, $R_1 = 10 k\Omega, R_f = 100 k\Omega, E_i = 10 V$  

### 3-8 Noninverting Adder  

Figure 3-14  

### 3-10 Difference Amplifiers  
#### 3-10.1 The Subtractor  

Figure 3-16  

## Chapter 05 : Selected Applications of Op Amps  
### 5.12 Integrators and Differentiators  

Figure 5-15  

#### 5-12.1 Integrators  

#### 5-12.3 Differentiators  

Figure 5-17  
Design Example 5-18  

## Chapter 11 : Active Filters  
### 11-0 Introduction  

Types  

### 11-1 Basic Low-Pass Filter  
#### 11-1.1 Introduction  

Figure 11-2  

#### 11-1.2 Designing the Filter  

Example 11-1, 11-2, 11-3  

#### 11-1.3 Filter Response  

$A_{CL}$  

### 11-5 High-Pass Butterworth Filters  
#### 11-5.1 Introduction  

Figure 11-6  

#### 11-5.1 20-dB/Decade Filter  

Figure 11-7  

---

> R. P. Jain - Modern Digital Electronics [4th Edition] (2012)  

## Chapter 10 : A/D and D/A Converters  
### 10.2 Digital-to-Analog Converters  
#### 10.2.1 Weighted-Resistor D/A Converter  

Fig. 10.1  

### 10.4 Sample-and-Hold  

Basic Idea of Sampling  

### 10.5 Analog-to-Digital Converters  
#### 10.5.1 Quantization and Encoding  

Basic Idea of Quantization and Encoding  

#### 10.5.2 Parallel-Comparator (or Flash) A/D Converter  

Fig. 10.19  

Use of DAC in Data Communication  

Nyquist  
- Sampling  
- Quantization (Levels)  
- Encoding  
